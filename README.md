...
- The `/resolve/:domain` and `/safe-resolve/:domain` routes require a full domain name ending in `.sns` or `.sol`, for example `mydomain.sns` or `mydomain.sol`.
...
- **Resolution:** `/resolve/:domain`, `/safe-resolve/:domain`
