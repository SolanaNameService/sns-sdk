...
import { assertMainnetDomainSupported } from "../src/utils/assertMainnetDomainSupported";
...
describe("TLD support", () => {
  ...
  test("accepts .sns without requesting the slot", async () => {
    ...
    await expect(
      assertMainnetDomainSupported(connection, "domain.sns"),
  ...
  test("rejects unsupported suffixes without requesting the slot", async () => {
    ...
    await expect(
      assertMainnetDomainSupported(connection, "domain.xyz"),
  ...
  test("does not cache pre-cutoff slots", async () => {
    ...
    await assertMainnetDomainSupported(connection, "domain.sol");
    await assertMainnetDomainSupported(connection, "domain.sol");
  ...
  test("permanently caches a post-cutoff endpoint", async () => {
    ...
    await expect(
      assertMainnetDomainSupported(connection, "domain.sol"),
    ).rejects.toThrow(UnsupportedTldError);
    await expect(
      assertMainnetDomainSupported(connection, "domain.sol"),
    ).rejects.toThrow(UnsupportedTldError);
  ...
  test("isolates the cache by RPC endpoint", async () => {
    ...
    await expect(
      assertMainnetDomainSupported(first.connection, "domain.sol"),
    ).rejects.toThrow(UnsupportedTldError);
    await expect(
      assertMainnetDomainSupported(second.connection, "domain.sol"),
    ).resolves.toBeUndefined();
