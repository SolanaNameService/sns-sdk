...
```toml
[dependencies]
sns-sdk = { version = "2.1", default-features = false, features = ["blocking"] }
solana-client = "2.1"
solana-sdk = "2.1"
```
...
| --------------------------------- | ---------------------------------------- | ------------------------------------------- |
| Resolution                        | Bare name or legacy `.sol` name          | Full `.sns` or `.sol` name                  |
| Record and sub-registrar reads    | Bare name or legacy `.sol` name          | Full lowercase `.sns` name                  |
...
`resolve` and `safe_resolve` now route `.sol` names through the SRS resolver. Record getters and sub-registrar lookup remain `.sns`-only and reject `.sol` domains.

:::

## Update resolution

Rename `resolve_owner` to `resolve`, pass a full `.sns` or `.sol` name, and add the required `AllowPda` argument:
...
:::info New API: `safe_resolve`

A new `safe_resolve` API compares SRS-backed `.sol` resolution with the corresponding `.sns` resolution and returns an address only when both targets match. It returns `SnsError::SnsSolResolutionMismatch` on mismatch.
...
Use `domain::get_sns_domains_for_owner` and `nft::get_sns_nfts_for_owner` to look up `.sns` domains/NFTs, and `domain::get_sol_domains_for_owner` and `nft::get_sol_nfts_for_owner` to look up `.sol` domains/NFTs:
...
.await?;
let tokenized = sns_sdk::non_blocking::resolve::get_tokenized_domains(
    &client,
    &owner,
)
.await?;
...
```rust title="v2"
let sns_domains = sns_sdk::non_blocking::domain::get_sns_domains_for_owner(
    &client,
    owner,
)
.await?;
let sns_nfts = sns_sdk::non_blocking::nft::get_sns_nfts_for_owner(
    &client,
    &owner,
)
.await?;

let sol_domains = sns_sdk::non_blocking::domain::get_sol_domains_for_owner(
    &client,
    owner,
)
.await?;
let sol_nfts = sns_sdk::non_blocking::nft::get_sol_nfts_for_owner(
    &client,
    &owner,
)
.await?;
...
The v2 `DomainKeyWithParent` also exposes `parent` and `is_sub`. To derive the key of a SRS-based `.sol` domain, use `get_sol_domain_key`.
...
V2 getters now borrow the RPC client, take the domain before the record, and require a full `.sns` domain:
