...
A new `useSafeResolve` hook has also been added. `useSafeResolve` only returns an address if the `.sol` domain and the corresponding `.sns` domain resolve to the same address — throwing an error on mismatch.
