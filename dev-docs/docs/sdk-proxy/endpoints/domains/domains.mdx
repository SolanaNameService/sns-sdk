...
`domain` is the TLD-trimmed `.sns` name without the `.sns` suffix. `key` is its name-account public key. Subdomains and entries without reverse-name data are omitted.
