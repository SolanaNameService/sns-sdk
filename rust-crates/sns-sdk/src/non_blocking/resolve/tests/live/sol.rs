...
#[tokio::test]
async fn resolves_sol_ip_5_fixtures_from_rpc() {
    ...
    for (domain, expected, allow_pda) in cases {
        let result = resolve(&client, domain, allow_pda).await.unwrap();
        assert_eq!(result, expected, "domain {domain}");
    }
}

#[tokio::test]
async fn returns_expected_sol_fixture_errors_from_rpc() {
    let client = live_client();

    let domain = "sns-ip-5-wallet-3.sol";
    assert!(matches!(
        resolve(&client, domain, AllowPda::Deny).await,
...
#[tokio::test]
async fn safe_resolves_sol_ip_5_fixtures_from_rpc() {
    let client = live_client();
    let system_program = solana_sdk_ids::system_program::ID;
    let cases = [
        (
    ...
    for (domain, expected, allow_pda) in cases {
        let result = safe_resolve(&client, domain, allow_pda).await.unwrap();
        assert_eq!(result, expected, "domain {domain}");
    }
}

#[tokio::test]
async fn returns_expected_safe_sol_fixture_errors_from_rpc() {
    let client = live_client();

    for domain in ["sns-ip-5-wallet-2.sol", "sns-ip-5-wallet-9.sol"] {
        assert!(matches!(
            safe_resolve(&client, domain, AllowPda::Deny).await,
