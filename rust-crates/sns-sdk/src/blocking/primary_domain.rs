...
mod account_tests {
    ...
    #[test]
    fn primary_domain_account_absent_returns_none() {
        let client = test_client("blocking-primary-domain-absent", None);

        assert_eq!(
            get_primary_domain(&client, &Pubkey::new_unique()).unwrap(),
            None
        );
    }

    #[test]
    fn primary_domain_account_valid_returns_name_account() {
        let name_account = Pubkey::new_unique();
        let account = primary_domain_account(4, name_account, NAME_OFFERS_PROGRAM_ID);
        let client = test_client("blocking-primary-domain-valid", Some(&account));
    ...
    #[test]
    fn primary_domain_account_wrong_owner_is_rejected() {
        let account = primary_domain_account(4, Pubkey::new_unique(), Pubkey::new_unique());
        let client = test_client("blocking-primary-domain-wrong-owner", Some(&account));
    ...
    #[test]
    fn primary_domain_account_truncated_data_is_rejected() {
        let account = Account {
            data: vec![4],
            owner: NAME_OFFERS_PROGRAM_ID,
            ..Account::default()
        };
        let client = test_client("blocking-primary-domain-truncated", Some(&account));
    ...
    #[test]
    fn primary_domain_account_wrong_tag_is_rejected() {
        let account = primary_domain_account(0, Pubkey::new_unique(), NAME_OFFERS_PROGRAM_ID);
        let client = test_client("blocking-primary-domain-wrong-tag", Some(&account));
