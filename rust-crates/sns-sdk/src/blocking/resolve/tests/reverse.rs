...
#[test]
fn resolve_reverse_with_parent_preserves_legacy_top_level_behavior() {
    ...
    let (legacy_client, legacy_sender) = test_client(
        "blocking-reverse-legacy",
        [(RpcRequest::GetAccountInfo, response.clone())],
    );
    let (parent_client, parent_sender) = test_client(
        "blocking-reverse-parent-none",
        [(RpcRequest::GetAccountInfo, response)],
    );

    let legacy = resolve_reverse(&legacy_client, &key).unwrap();
    let parent_aware = resolve_reverse_with_parent(&parent_client, &key, None).unwrap();
...
#[test]
fn resolve_reverse_with_parent_accepts_unmarked_child_payload() {
    let key = Pubkey::new_unique();
    let parent = Pubkey::new_unique();
    let account = reverse_account(&encoded_reverse(b"team"));
    let (client, _) = test_client(
        "blocking-reverse-child-unmarked",
        [(RpcRequest::GetAccountInfo, account_response(Some(&account)))],
    );
...
#[test]
fn resolve_reverse_with_parent_returns_none_for_absent_account() {
    let (client, _) = test_client("blocking-reverse-absent", []);

    assert_eq!(
        resolve_reverse_with_parent(&client, &Pubkey::new_unique(), None).unwrap(),
        None
    );
}

#[test]
fn resolve_reverse_with_parent_rejects_malformed_payloads() {
    let payloads = [
        vec![1, 0, 0],
    ...
    for (index, payload) in payloads.iter().enumerate() {
        ...
        let (client, _) = test_client(
            &format!("blocking-reverse-malformed-{index}"),
            [(RpcRequest::GetAccountInfo, account_response(Some(&account)))],
        );
        assert!(matches!(
